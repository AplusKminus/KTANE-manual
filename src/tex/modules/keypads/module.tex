...
\newcommand{\keypadimg}[1]{\vspace{0.3cm}\includegraphics[width=1.9cm,height=1.9cm,keepaspectratio]{symbols/#1}}
...
\newcommand{\squidknife}{\keypadimg{squidknife.png}}       % CYRILLIC CAPITAL LETTER IOTIFIED BIG YUS
...
\begin{bulletlist}
  \bulletitem{Only one column below has all four of the symbols from the keypad.}
  \bulletitem{Press the four buttons in the order their symbols appear from top to bottom within that column.}
  \bulletitem{The framed symbols on white occur only once each.}
  \bulletitem{The colored symbols each occur a second time in a distant position.}
  \bulletitem{The gray symbols each occur a second time in a nearby position.}
\end{bulletlist}
...
\begin{NiceTabular}[cell-space-top-limit=0pt, cell-space-bottom-limit=0pt]{
  ...
  \Body
    ...
    \squigglyn &  & \cursive &  & \doublek &  & \rightc &  & \squidknife &  & \aesymbol \\
    \squidknife &  & \hollowstar &  & \meltedthree &  & \paragraphsymbol &  & \doublek &  & \pitchforksymbol \\
